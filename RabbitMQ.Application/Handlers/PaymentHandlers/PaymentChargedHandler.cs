...
namespace RabbitMQ.Application.Handlers.PaymentHandlers
{
    public class PaymentChargedHandler : IMessageHandler
    {
        ...
        public async Task HandleAsync(JsonElement payload, Guid messageId)
        {
            ...
            var evt = payload.Deserialize<PaymentCharged>()
                ?? throw new InvalidOperationException($"Failed to deserialize {nameof(PaymentCharged)}");

            // for testing
            _logger.LogInformation("Handling {MessageType} for saga {CorrelationId}", nameof(PaymentChargedHandler), evt.CorrelationId);
